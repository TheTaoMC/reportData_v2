...
export const useStore = create((set, get) => ({
    ...
    zuFetchMaster: async () => {
        try {
            ...
            const responseCustomers = await fetch(get().zu_Url_Base + "customer/read.php",
                {
                    method: "GET",
                    headers: {
            ...
            const responseProducts = await fetch(get().zu_Url_Base + "product/read.php",
                {
                    method: "GET",
                });
            const responseWeighttypes = await fetch(get().zu_Url_Base + "weighttype/read.php",
                {
                    method: "GET",
                });
            const responseDrivers = await fetch(get().zu_Url_Base + "driver/read.php",
                {
                    method: "GET",
                });
            const responseTransporters = await fetch(get().zu_Url_Base + "transporter/read.php",
                {
                    method: "GET",
                });


            if (!responseCustomers.ok || !responseProducts.ok || !responseWeighttypes.ok || !responseDrivers.ok || !responseTransporters.ok) {
                //set({ zu_Data: [] });
                throw new Error(`HTTP error! Status: ${responseCustomers.status, responseProducts.status, responseWeighttypes.status, responseDrivers.status, responseTransporters.status}`);
            }
